...
echo "$($PSQL "SELECT distinct course FROM courses WHERE course<'D'")"
...
echo "$($PSQL "SELECT DISTINCT(major) FROM students RIGHT JOIN majors ON students.major_id=majors.major_id WHERE students.major_id IS NULL OR first_name ILIKE '%ma%' ORDER BY major")"
